...
public class TurtlesGameCanvas extends GameCanvas {
	...
	private static final Color BACKGROUND_LCD_COLOR = new Color(240, 253, 220);
    ...
    {
        super();
        super.setBackgroundColor(BACKGROUND_LCD_COLOR);
        ...
        System.out.println("Error! Failed to register font digital-7");
        e.printStackTrace();
       }
	...
	private void checkIfPlayerInRiver() {
		...
		Sprite sprite = (Sprite) spritesArray[1][getPlayer()
				.getCurrentPosition()];
	...
	protected void showIntroScreen(Graphics2D g) {
		g.setColor(BACKGROUND_LCD_COLOR); // rough approx of b/w LCD background
											// color
		...
		g.drawString(
				"Carry your luggage from one side of the river to the other by",
				110, 70);
		g.drawString(
				"jumping on the turtles backs. Avoid falling in the water when",
				110, 90);
		...
		g.drawString("2015 Kevin Hooke (@kevinhooke)", 110, 240);
	...
	protected void calculatePlayerPosition(PlayerSprite player) {
		...
		if (riverCrosser.isPlayerMovingLeft() && ableToMoveLeft(riverCrosser)
				&& !riverCrosser.isPlayerInRiver()) {
			riverCrosser.moveLeft();
		}
		if (riverCrosser.isPlayerMovingRight() && ableToMoveRight(riverCrosser)
				&& !riverCrosser.isPlayerInRiver()) {
	...
	/**
	 * Calculates fish visible and the frequency based on current score - the
	 * higher the current score results in a higher probability that a fish will
	 * appear and swim up to be eaten.
	 */
	...
	private void calculateTurtlePositions() {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (fish.getPosition() == FishSprite.TURTLE_FOOD) {
				...
				// TODO: change this from bottom_river_pos to lowest - check
				// this works
